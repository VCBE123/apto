...
SEED = 42
df_train = pd.read_csv('/data/wen/data/aptos/trainLabels.csv')
df_val = pd.read_csv('/data/wen/data/aptos/train.csv')
# df_test = pd.read_csv('/data/wen/data/aptos/test.csv')
train_x = df_train['image']
train_y = df_train['level']
...
valid_x=df_val['id_code']
valid_y=df_val['diagnosis']
train_transform = Compose([
	Resize(320),
	RandomResizedCrop(300),
	RandomAffine(degrees=2, translate=(0.02, 0.02), scale=(0.98, 1.02), shear=2, fillcolor=0),
...
def get_data():
	train_set = ImageDataset(root='/data/wen/data/aptos/resized_train/',extension='.jpeg', path_list=train_x, target=train_y,
							 transform=train_transform)
	train_eval_set = ImageDataset(root='/data/wen/data/aptos/train_images/', path_list=valid_x, target=valid_y,
	...
	train_loader = data.DataLoader(train_set, batch_size=train_batch_size, num_workers=num_workers, shuffle=True,
								   drop_last=True, pin_memory=True)
	eval_loader = data.DataLoader(train_eval_set, batch_size=eval_batch_size, num_workers=num_workers, shuffle=False,
								  drop_last=False, pin_memory=True)
	# test_loader = data.DataLoader(test_set, batch_size=eval_batch_size, num_workers=num_workers, shuffle=False,
	# 							  drop_last=False, pin_memory=True)
